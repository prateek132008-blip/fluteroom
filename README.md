...
A premium, mobile-first, dark-themed enrollment site for live 1-on-1
online flute classes. Static HTML/CSS/JS — no backend server. Form
submissions are saved to Google Sheets through a free Google Apps
Script Web App.

👉 **New here? Read `SETUP_GUIDE.md` first** — it's the simple,
step-by-step version of everything below, written for a non-technical
person.
...
script.js               All site logic (form, pixel events, Instagram embed)
config.js                ⭐ Every editable setting lives here
SETUP_GUIDE.md           Plain-language setup walkthrough
...
before launch (exact filenames below). Student testimonials are
text-only by design — no review photos are used anywhere on the site.
...
2. Open the site and confirm:
   ...
   - **ViewContent** fires once you scroll the "What You Will Learn" (curriculum) section into view, or click "View Curriculum PDF."
...
### 7b. Test the Instagram Reel embed
1. Open the site and scroll to the teacher section.
2. The Reel should load inline. On a slow connection or if Instagram's
   script is blocked, a "Watch on Instagram ↗" button appears instead
   after a few seconds — this is the intended fallback, not a bug.
...
### 10. Update testimonials as you collect more
In `index.html`, find `SECTION 7 — STUDENT REVIEWS`. Three real,
text-only reviews are already in place. Edit the name, level, and
review text directly in each `.review-card` block whenever you want
to swap in newer testimonials. No review photos are used by design.
...
| ViewContent | Visitor scrolls the curriculum ("What You Will Learn") section into view, clicks "View Curriculum PDF", or plays the hero sneak-peek video (each is a separate, once-only signal) |
